...
void fused_residual_rmsnorm_forward(
    ...
    switch(kernel_num){
        case 1:
            fused_residual_rmsnorm_forward1(y, z, mean2, x, residual, w, B, T, C, block_size);
            break;
        case 2:
            fused_residual_rmsnorm_forward2(y, z, mean2, x, residual, w, B, T, C, block_size);
            break;
        case 3:
            fused_residual_rmsnorm_forward3(y, z, mean2, x, residual, w, B, T, C, block_size);
            break;
        case 4:
            fused_residual_rmsnorm_forward4(y, z, mean2, x, residual, w, B, T, C, block_size);
            break;
        case 5:
            fused_residual_rmsnorm_forward5(y, z, mean2, x, residual, w, B, T, C, block_size);
            break;
        case 6:
            fused_residual_rmsnorm_forward6(y, z, mean2, x, residual, w, B, T, C, block_size);
            break;
        default:
...
int main(int argc, char **argv){
    ...
    CUDA_CHECK(cudaMalloc(&d_residual, B * T * C * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_w, C * sizeof(float)));

    CUDA_CHECK(cudaMemcpy(d_x, x, B * T * C * sizeof(float), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_residual, residual, B * T * C * sizeof(float), cudaMemcpyHostToDevice));
    ...
    for(int j = 0; j < sizeof(block_sizes) / sizeof(int); ++j){
        ...
        fused_residual_rmsnorm_forward(kernel_num, d_y, d_z, d_mean2, d_x, d_residual, d_w, B, T, C, block_size);

        validate_result(d_y, y, "y", B * T * C, 1e-5f);
        validate_result(d_z, z, "z", B * T * C, 1e-5f);
